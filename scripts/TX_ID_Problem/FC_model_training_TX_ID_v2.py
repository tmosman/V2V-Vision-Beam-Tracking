...
def Train_model(datasets_dir,output_dir,nn, optimizer):
    ...
    val_save_path = f'./{output_dir}/scenario36_val_DistAngle_Exp1.csv'
    ...
    y_orig = df[['gc_dist_norm','gc_angle_norm']]
    ...
    for e in  range(1, num_epochs+1):   
        ...
        for tr_count, (pwr_data, center_data) in enumerate(train_loader):
            ...
            label = center_data[:,].type(torch.Tensor)  
        ...
        with torch.no_grad():
            ...
            for tr_count, (pwr_data, center_data) in enumerate(val_loader):
                data = pwr_data.type(torch.Tensor)   
                #print(pwr_data, center_data)  # Index  0 --> power vector , 1 -->  (dist,angle)            
                label = center_data[:, ].type(torch.Tensor)                      
...
def main():
    trained_model = Train_model(datasets_dir='Annotated_Datasets',output_dir='Output_Datasets',nn=nn,optimizer=optimizer)
