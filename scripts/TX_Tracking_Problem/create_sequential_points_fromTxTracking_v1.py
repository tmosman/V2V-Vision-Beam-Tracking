...
def polar_dist(pos1,pos2):
    r1,r2 = pos1[0],pos2[0]
    t1,t2 = math.radians(pos1[1]),math.radians(pos2[1])
    d = np.sqrt(r1**2+r2**2 - 2*r1*r2*np.cos(t1-t2))
    return d



df = pd.read_csv(r'./TX_ID_Outputs/scenario36_teston_seq_1_cleaned_Nov_Pred_dist_Angle_run1.csv')
csv_to_save = r'./TX_Tracking/Tx_Tracking_output_Seq_1_Nov_run1.csv'
    ...
    for iteration in range(0,6):
        ...
        if len(dist)>= 1:
            ...
            for q,(dis,ang) in enumerate(zip(dist,angle)):
                min_d =polar_dist([pred_dist,pred_angle],[dis,ang])
