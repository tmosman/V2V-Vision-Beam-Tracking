...
class LSTM_class():
    ...
    def create_structure(self,input_dim,n_timesteps_in,n_timesteps_out,num_hidden_nodes,n_features):
        self.lstm_model.add(LSTM(num_hidden_nodes, input_shape=(n_timesteps_in,input_dim)))
        self.lstm_model.add(RepeatVector(n_timesteps_out))
        self.lstm_model.add(Dropout(0.5))
        self.lstm_model.add(LSTM(num_hidden_nodes, return_sequences=True))
        self.lstm_model.add(Dropout(0.5))
        self.lstm_model.add(LSTM(num_hidden_nodes, return_sequences=True))
        self.lstm_model.add(TimeDistributed(Dense(n_features, activation='softmax')))
